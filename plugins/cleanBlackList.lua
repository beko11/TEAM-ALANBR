...
if not is_sudo(msg) then
    ...
    local function cleanbl(ext, res)
      if tonumber(res.total_count_) == 0 then -- «Blocklist is empty or maybe Bot is not group's admin»
        return tdcli.sendMessage(ext.chat_id, ext.msg_id, 0, '⚠️ _ 📌¦قائمه الحظر خاليه او ان البوت ليس ادمن _ !', 1 , 'md')
      end
      local x = 0
      ...
      return tdcli.sendMessage(ext.chat_id, ext.msg_id, 0, '✅ _ 📌¦ تم تنظيف قائمه الحظر _ !', 1 , 'md')
    end
